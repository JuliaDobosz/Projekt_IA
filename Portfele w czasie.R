...
cl <- makeCluster(4)  # Wybierz liczbę rdzeni
...
#wykres wag dla pojedynczego portfela
chart.Weights(pmr_zmienne)
chart.Weights(pme_zmienne)

#oglądanie wag
extractWeights(pmr_zmienne)
extractWeights(pme_zmienne)

#oglądanie wyników optymalizacji
extractObjectiveMeasures(pmr_zmienne)
extractObjectiveMeasures(pme_zmienne)

rr <- Return.portfolio(R = portfel.xts, weights = extractWeights(pmr))
charts.PerformanceSummary(rr)
re <- Return.portfolio(R = portfel.xts, weights = extractWeights(pme))
charts.PerformanceSummary(re)
